...
export default function AccountPage() {
  ...
  const [twoFactorValidation, setTwoFactorValidation] = useState('')
  const [twoFactorRecoveryKeys, setTwoFactorRecoveryKeys] = useState([])
  ...
  useEffect(() => {
    async function loadPage() {
      ...
      setDisabled(operationsEnabled === false)
  ...
  async function changeEmail() {
    const userAlreadyExists = await api.post('api/v1/account/exists', { email: newEmail }, {
      redirectOnUnauthorized: false,
      showToast: false,
    })
    ...
    await api.post('manage/info', { newEmail }, {
      redirectOnUnauthorized: false,
      showToast: true,
    })
  ...
  async function changePassword() {
    ...
    await api.post('manage/info', { oldPassword, newPassword }, {
      redirectOnUnauthorized: false,
      showToast: true,
    })
  ...
  async function resetRecoveryCodes() {
    ...
    if (response?.recoveryCodes?.length > 0) {
      setTwoFactorRecoveryKeys(response.recoveryCodes)
    }
  }
  ...
  return (
    <>
      ...
      <Row className="mt-3 g-3 align-items-start">
            ...
            <Card.Body>
              ...
              <fieldset disabled={disabled}>
                <Form.Group className="mb-3" controlId="accountNewEmail">
                  <Form.Control value={newEmail} onChange={(event) => setNewEmail(event.target.value)} />
                </Form.Group>
                <div className="text-end">
                  <Button onClick={() => openConfirmation('Change Email', 'Please confirm you want to change your email.', changeEmail)}>
                    Save Email
                  </Button>
                </div>
              </fieldset>
          ...
          <Card className="feature-card">
            ...
            <Card.Body>
              <Form.Group className="mb-3" controlId="accountOldPassword">
                <Form.Control type="password" placeholder="Old Password" value={oldPassword} onChange={(event) => setOldPassword(event.target.value)} />
              </Form.Group>
              <Form.Group className="mb-3" controlId="accountNewPassword">
                <Form.Control type="password" placeholder="New Password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} />
              </Form.Group>
              ...
              <div className="text-end">
                <Button onClick={() => openConfirmation('Change Password', 'Please confirm you want to change your password.', changePassword)}>
                  Save Password
                </Button>
              </div>
            </Card.Body>
          ...
          <Card className="feature-card">
            ...
            <Card.Body>
              {twoFactorEnabled ? (
                <>
                  <h5>
                    Two Factor Authentication is <span className="status-enabled">ENABLED</span>
                  </h5>
                  <div className="d-flex flex-wrap gap-2 mt-3">
                    <Button variant="danger" onClick={disableTwoFactor}>Disable 2FA</Button>
                    <Button variant="warning" onClick={forgetMachine}>Forget this PC</Button>
                  </div>
                  {twoFactorRecoveryKeys.length > 0 ? (
                    <div className="mt-4 text-center">
                      <p>Print or save these recovery keys. You cannot retrieve them after leaving this page.</p>
                      {twoFactorRecoveryKeys.map((code) => (
                        <h6 key={code}>{code}</h6>
                      ))}
                    </div>
                  ) : null}
                ...
                <div className="text-center">
                  ...
                  <Form.Group className="mb-3" controlId="twoFactorValidation">
                    ...
                    <Form.Control value={twoFactorValidation} onChange={(event) => setTwoFactorValidation(event.target.value)} />
                  </Form.Group>
                  <Button className="w-100" onClick={finishTwoFactorSetup}>Submit Code and Finish Setup</Button>
                </div>
              ) : (
                <>
                  <h5>
                    Two Factor Authentication is <span className="status-disabled">DISABLED</span>
                  </h5>
                  <Button className="mt-3" onClick={startTwoFactorSetup}>Enable 2FA</Button>
                  <p className="mt-3 mb-1">Use an authenticator app to get a security code when signing in.</p>
                  <a href="https://www.microsoft.com/en-us/security/mobile-authenticator-app" target="_blank" rel="noreferrer">
        ...
        <Col xs={12} md={6} xl={4}>
          <Card className="feature-card border-danger">
            ...
            <Card.Body>
              ...
              <Button variant="danger" onClick={() => openConfirmation('Delete Account', 'Please confirm you want to delete your account. This cannot be undone.', deleteAccount, 'Delete')}>
                Delete Account
              </Button>
            </Card.Body>
          </Card>
